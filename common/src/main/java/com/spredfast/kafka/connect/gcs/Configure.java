...
public abstract class Configure {
	...
	private static final Class<? extends GCSRecordFormat> DEFAULT_FORMAT = TrailingDelimiterFormat.class;
	...
	public static Converter buildConverter(Map<String, ?> props, String classNameProp, boolean isKey, Class<? extends Converter> defaultConverterClass) {
		...
		try {
			...
			throw new IllegalArgumentException("Could not create GCS converter for props " + classNameProp + " isKey=" + isKey, e);
		}
	}
	...
	public static Metrics metrics(Map<String, String> props) {
		return ofNullable(props.get("metrics.reporter"))
			.map(className -> className.equals("datadog") ?
				"com.spredfast.kafka.connect.gcs.metrics.DatadogMetrics" : className)
	...
	public static GCSRecordFormat createFormat(Map<String, String> props) {
		try {
			GCSRecordFormat recordFormat = (GCSRecordFormat) ofNullable(props.get("format")).map(Object::toString)
